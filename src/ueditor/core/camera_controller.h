		...
		void on_update() override {
			world().view<Transform, Camera, CameraController>().each([](Transform& transform, Camera& camera, CameraController& camera_controller) {
				...
				if (Input::is_mouse_button(UENGINE_MOUSE_BUTTON_RIGHT)) {
					...
					camera_controller.yaw += mouse_position_delta.x * camera_controller.sensitivity * Time::delta();
				}

				transform.rotation = Float4::quaternion(Float3(0.0f, camera_controller.yaw, 0.0f)) 
					* Float4::quaternion(Float3(camera_controller.pitch, 0.0f, 0.0f));
			});
		}
	};
}
